...
# Začetek MLflow teka
with mlflow.start_run(run_name="OvrednotenjeModela2"):
    ...
    'Free flow travel time',
    'Confidence',]),
        ('scaler1', StandardScaler(), ['Average speed']),
    ])
    ...
    #mlflow.set_tag("tag", "production2")
    #mlflow.register_model("runs:/" + mlflow.active_run().info.run_id + "/lstm_model", "OvrednotenjeModela2")
    ...
    last_run = mlflow.search_runs(filter_string="tags.tag='production2'",
    ...
    if mse < mse_prod:
        mlflow.set_tag("tag", "production2")
        mlflow.register_model("runs:/" + mlflow.active_run().info.run_id + "/lstm_model", "Production2")
    else:
        mlflow.set_tag("tag", "test2")
        mlflow.register_model("runs:/" + mlflow.active_run().info.run_id + "/lstm_model", "Test2")
